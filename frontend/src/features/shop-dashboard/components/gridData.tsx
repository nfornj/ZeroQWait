...
export interface DashboardGridColumn {
  ...
  renderCell?: (value: unknown, row: DashboardGridRow) => ReactNode;
}
...
function renderSparklineCell(value: unknown) {
  if (!Array.isArray(value) || value.length === 0) {
    return null;
  }

  const data = (value as SparkLineData).map((entry, index) => ({ index, value: entry }));
...
export function renderAvatar(value: { name: string; color: string } | null) {
  if (value == null) {
    return '';
  }

  return (
    <Avatar className="size-6" style={{ backgroundColor: value.color }}>
...
export const columns: DashboardGridColumn[] = [
  { field: 'pageTitle', headerName: 'Page Title' },
  {
    field: 'status',
    headerName: 'Status',
    renderCell: (value) => renderStatus(value as "Online" | "Offline"),
  },
  {
    field: 'users',
    headerName: 'Users',
...
export const rows: DashboardGridRow[] = [
  {
    id: 1,
