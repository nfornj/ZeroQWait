...
async def send_message_with_buttons(
    ...
    """Send a message with an InlineKeyboard.  Returns True on success.

    ``buttons`` is a 2-D list of Telegram InlineKeyboardButton dicts:
        [[{"text": "✅ Approve", "callback_data": "approve:uuid"}, ...], ...]
    """
    if not TELEGRAM_BOT_TOKEN:
        logger.debug("TELEGRAM_BOT_TOKEN not set — skipping send_message_with_buttons")
        return False
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
        ...
        logger.error("Telegram send_message_with_buttons error: %s", exc)
        return False
...
async def set_webhook(webhook_url: str) -> bool:
    ...
    Includes ``callback_query`` in allowed_updates so inline button taps
    (HITL escalations) are delivered to the webhook.
    """
    if not TELEGRAM_BOT_TOKEN:
        return False
    try:
